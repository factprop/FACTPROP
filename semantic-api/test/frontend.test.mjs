...
test('file loading does not send and resets consent',async()=>{const s=setup();await s.agree();await s.$('semantic-file').emit('change',{target:{files:[{name:'sample.json',size:80,text:async()=>'[{"text":"Apple","privateMetadata":"excluded"}]'}]}});assert.equal(s.calls.length,0);assert.equal(s.$('semantic-input').value,'Apple');assert.equal(s.$('semantic-consent').checked,false);});
test('flexible dataset extension loads for review',async()=>{const s=setup();await s.$('semantic-file').emit('change',{target:{files:[{name:'sample.tsv',size:30,text:async()=>'Apple Inc.\\nParis'}]}});assert.equal(s.calls.length,0);assert.equal(s.$('semantic-input').value,'Apple Inc.\\nParis');});
